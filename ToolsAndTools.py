...
def sales_admin_workshop_data(user_role, supervisor_code):
    ...
    if user_role == "Super Admin":
        ...
        df = pd.read_sql_query(
            "SELECT * FROM Workstation_Data WHERE supervisor_name = %s", conn, params=(supervisor_code,)
        )
...
def workstation_entry_by_supervisor(supervisor_code):
    ...
    c.execute("SELECT name FROM User_Credentials WHERE Supervisor_Code = %s AND User_Role = 'Workstation'", (supervisor_code,))
    wksts = c.fetchall()
        ...
        for row in rows_data:
            ...
            if record_exists:
                ...
                c.execute('''
                    ...
                    SET running_repair = %s, free_service = %s, timestamp = %s, paid_service = %s, body_shop = %s, total = %s, align = %s, balance = %s, align_and_balance = %s
                    WHERE date = %s AND workstation_name = %s
                ''', (
...
def sales_admin_workshop_report(user_role, supervisor_code):
    """View workshop report filtered by Supervisor and include Target as a virtual column."""
    st.subheader("Workshop Report")
    
    # Get user role and supervisor code from session state
    user_role = st.session_state.user_data['role']
    supervisor_code = st.session_state.user_data.get('code')

    conn = mysql.connector.connect(**mydb())

    # Fetch workstation data based on user role
    if user_role == "Super Admin":
        df = pd.read_sql_query("SELECT * FROM Workstation_Data", conn)
    elif user_role == "Supervisor":
        df = pd.read_sql_query(
    ...
    # Fetch Target values from User_Credentials for workstations
    target_df = pd.read_sql_query(
        "SELECT Name AS workstation_name, Target FROM User_Credentials WHERE User_Role = 'Workstation'", conn
    )
    ...
    # Merge Workstation_Data with Target values (as a virtual column)
    df = df.merge(target_df, on="workstation_name", how="left")
    ...
    if start_date and end_date:
        ...
        if filtered_df.empty:
            ...
            # Summarize Data
            summary = filtered_df.groupby(['workstation_name', 'Target']).agg(
...
# =====================================================================
# Chart will not display until the target fillup & the necessary data filledup
def sales_admin_workshop_chart(user_role, supervisor_code):
    """View workshop chart filtered by Supervisor, grouped by month, and create grouped bar charts."""
    st.subheader("Workshop Chart")
    ...
    # Merge Workstation_Data with Target values
    df = df.merge(target_df, on="workstation_name", how="left")
    ...
    if start_date and end_date:
        ...
        # Select Workstation
        workstation_list = df_filtered["workstation_name"].unique().tolist()
        selected_workstation = st.selectbox("Select Workstation", ["All"] + workstation_list)

        # Hide everything if "All" is selected
        if selected_workstation == "All":
            return  # Stop execution immediately if "All" is selected

        # Filter Data for Selected Workstation
        df_filtered = df_filtered[df_filtered["workstation_name"] == selected_workstation]

        # Extract Month name from Date
        df_filtered['Month'] = df_filtered['date'].dt.strftime('%B')  # e.g., 'April'
        ...
        summary = df_filtered.groupby(['Month']).agg(
            Align=('align', 'sum'),
            Balance=('balance', 'sum'),
            Total=('align_and_balance', 'sum'),
            PMS=('paid_service', 'sum'),
            Target=('Target', 'mean')
        ).reset_index()
        ...
        # Show Table & Charts If Data Exists
        if not summary.empty:
            ...
            categories = ['Align', 'Balance', 'Total', 'PMS', 'Target']
            x = np.arange(len(categories))  # Bar positions

            for i, month in enumerate(summary["Month"].unique()):
                month_data = summary[summary["Month"] == month].iloc[0]

                st.write(f"### {month}")  # Display Month Header

                # Create Matplotlib figure
                plt.figure(figsize=(8, 5))

                # Plot bars for different metrics
                plt.bar(x, [month_data[c] for c in categories], color=['blue', 'orange', 'green', 'red', 'purple'])

                # Labels and Titles
                plt.xlabel("Metrics")
                plt.ylabel("Values")
                plt.xticks(x, categories, rotation=45)
                plt.title(f"Performance Report - {month}")
                plt.grid(axis='y', linestyle="--", alpha=0.7)

                # Show values on top of bars
                for index, value in enumerate([month_data[c] for c in categories]):
                    plt.text(index, value + 5, str(int(value)), ha='center', fontsize=10, fontweight='bold')
...
def advisor_admin_workshop_data(user_role, supervisor_code):
    ...
    if user_role == "Super Admin":
        ...
        df = pd.read_sql_query(
            "SELECT * FROM Advisor_Data WHERE supervisor_name = %s", conn, params=(supervisor_code,)
        )
    else:
        st.error("Unauthorized access")
        return
...
def authenticate_user(code, password):
    ...
    # connection = conn()       # <-- add the parentheses
    c = conn.cursor()
    c.execute('SELECT * FROM User_Credentials WHERE Code = %s AND Password = %s', (code, password))
    user = c.fetchone()
    conn.close()
    return user
...
def fetch_supervisor_name(code):
    ...
    c = conn.cursor()
    c.execute('SELECT Supervisor_Name FROM User_Credentials WHERE Code = %s', (code,))
...
def has_in_time_recorded_today(code):
    ...
    c.execute('SELECT * FROM Attendance WHERE Code = %s AND Attendance_Date = %s', (code, today_date))
...
def fetch_supervisor_name(code):
    ...
    c.execute('SELECT Supervisor_Code FROM User_Credentials WHERE Code = %s', (code,))
    supervisor_code = c.fetchone()[0]

    # Now, fetch the Name of the supervisor where the Code matches the Supervisor_Code
    c.execute('SELECT Name FROM User_Credentials WHERE Code = %s', (supervisor_code,))
    supervisor_name = c.fetchone()[0]
...
def technician_data():
    ...
    attendance_date = datetime.now(ist).strftime("%Y-%m-%d")
    ...
    if not has_in_time_recorded_today(user_data['code']):
        ...
        if out_photo:
            ...
            c = conn.cursor()
            c.execute('SELECT In_Time FROM Attendance WHERE Code = %s AND Attendance_Date = %s', (user_data['code'], attendance_date))
...
def generate_attendance_report(start_date, end_date):
    ...
    mask = (df['Attendance_Date'] >= pd.to_datetime(start_date)) & (df['Attendance_Date'] <= pd.to_datetime(end_date))
    df = df.loc[mask]
...
def display_admin_report():
    ...
    if start_date > end_date:
        ...
        if st.button("Generate Report"):
            generate_attendance_report(start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"))
#---------------------------------------------------------


def download_all_reports():
    """Download all MySQL tables into one Excel file (multiple sheets)."""
    try:
        db_config = mydb()  # Your custom function from secrets
        ...
        output.seek(0)  # Move to beginning of the buffer
        return output.read()

    except Exception as e:
        st.error(f"Failed to export reports: {e}")
        return None
...
def manage_super_admin_data():
    ...
    if menu == "Download All Reports":
        excel_data = download_all_reports()

        if excel_data:
            st.download_button(
                label="📥 Download All Reports (Excel)",
                data=excel_data,
                file_name="All_MySQL_Tables.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )

            
    elif menu == "Sales Admin":
        sub_menu = st.sidebar.radio("Sub Options", ["Workshop Data", "Workshop Report", "Workshop Chart"])
        ...
        elif sub_menu == "Workshop Chart":
            sales_admin_workshop_chart(user_role, supervisor_code)
...
def manage_Supervisor_data():
    ...
    if menu == "Download All Reports":
        ...
        with tab1:
            ...
            user_code = st.session_state.user_data['code']
            conn = mysql.connector.connect(**mydb())
            user_df = pd.read_sql_query("SELECT * FROM User_Credentials WHERE Supervisor_Code = %s", conn, params=(user_code,))
            conn.close()
...
def mark_holiday():
    ...
    technicians_query = '''
        ...
        FROM Attendance a
        JOIN User_Credentials u ON a.Code = u.Code
        WHERE u.Supervisor_Code = %s AND a.Shift_Duration IS NOT NULL
        ORDER BY u.Name ASC
    '''
    ...
    FROM Attendance
    WHERE Code = %s AND Shift_Duration IS NOT NULL
    ...
    if mark_holiday:
        ...
        if st.button("Mark as Holiday"):
            ...
            cursor.execute(
                '''
                UPDATE Attendance
                SET Holiday = 1, Holiday_Remarks = %s
                WHERE Code = %s AND Attendance_Date = %s
                ''',
...
def mark_attendance():
    ...
    technicians_query = '''
        ...
        WHERE Supervisor_Code = %s AND User_Role = "Technician"
        ORDER BY Name ASC
    '''
    technicians = pd.read_sql_query(technicians_query, conn, params=(supervisor_code,))
    if technicians.empty:
    ...
    SELECT Name, Code
    FROM User_Credentials
    WHERE Supervisor_Code = %s AND User_Role = "Workstation"
    ORDER BY Name ASC
    """
        ...
        try:
            ...
            cursor.execute(
                    ...
                    ON DUPLICATE KEY UPDATE
                        In_Time = IFNULL(VALUES(In_Time), In_Time),
                        In_Time_Photo_Link = IFNULL(VALUES(In_Time_Photo_Link), In_Time_Photo_Link)
                    ''',
            ...
            try:
                ...
                cursor.execute(
                    ...
                    INSERT INTO Attendance (
                        Code, Name, Workstation_Name, Attendance_Date, In_Time, Out_Time,
                        Shift_Duration, In_Time_Photo_Link, Out_Time_Photo_Link, Supervisor_Name
                    )
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE
                        In_Time = IFNULL(VALUES(In_Time), In_Time),
                        Out_Time = IFNULL(VALUES(Out_Time), Out_Time),
                        Shift_Duration = IFNULL(VALUES(Shift_Duration), Shift_Duration),
                        In_Time_Photo_Link = IFNULL(VALUES(In_Time_Photo_Link), In_Time_Photo_Link),
                        Out_Time_Photo_Link = IFNULL(VALUES(Out_Time_Photo_Link), Out_Time_Photo_Link)
                    ''',
                    (
                        technician_code, technician_name, workstation_name, past_date,
                        past_in_time, past_out_time, shift_duration,
                        logged_in_name, logged_in_name, supervisor_name
                    )
                )
                conn.commit()
...
def enable_past_attendance():
    if st.session_state.user_data['name'] == "Amit":  # Only for Amit
        ...
        if st.button("Save Settings"):
            ...
            cursor.execute("INSERT INTO Past_Attendance (Status, Days) VALUES (%s, %s)", 
...
def fetch_technicians(supervisor_code):
    ...
    connection = conn()       # <-- add the parentheses
    c = connection.cursor()
    c.execute('SELECT Code, Name FROM User_Credentials WHERE Supervisor_Code = %s', (supervisor_code,))
...
def display_supervisor_report(): 
    ...
    if start_date > end_date: 
        ...
        if st.button("Generate Report"): 
            ...
            generate_sv_attendance_report(start_date.strftime("%Y-%m-%d"), end_date.strftime("%Y-%m-%d"), sname)
...
def fetch_name(code):
    ...
    # connection = conn()       # <-- add the parentheses
    c = conn.cursor()
    c.execute('SELECT name FROM User_Credentials WHERE Code = %s', (code,))
    sname = c.fetchone()[0]
    conn.close()
    return sname
...
def generate_sv_attendance_report(start_date, end_date, sname):
    ...
    df['Attendance_Date'] = pd.to_datetime(df['Attendance_Date'], format='%Y-%m-%d', errors='coerce')
    df['Shift_Duration'] = pd.to_timedelta(df['Shift_Duration'], errors='coerce')
